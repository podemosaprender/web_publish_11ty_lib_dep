...
P_SITE_JSON_DIR='src/sites_json';
P_VAR_DIR='xvar'
BASE_DIR=`cd ${0%/*}/../../.. ; pwd`
echo $BASE_DIR
...
for i in $P_SITE_JSON_DIR/*.json ; do
	...
	files_json_name="$P_VAR_DIR/files_json/$iname"
	...
	h256before=`cat $P_VAR_DIR/files_json/${iname%%json}sha256 2>/dev/null`
	#DBG:echo "$h256now"; echo "$h256before"
	if [ "$h256now" == "$h256before" ]; then
		echo "$i NOT changed"
	else
		echo "$i changed, generate"
		echo "$h256now" > $P_VAR_DIR/files_json/${iname%%json}sha256
		#A: sha256 updated
